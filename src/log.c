...
{
	snprintf(logdata->buffer, LOGLINE_MAXLEN, "%s -!- %s has joined %s",
		 timestamp(), ircmask, channel);
...
void log_quit(log_t *logdata, const char *ircmask, const char *channel,
	      const char *message)
{
	snprintf(logdata->buffer, LOGLINE_MAXLEN, "%s -!- %s has quit [%s]",
	...
	if (strlen(message) > 8
		...
		*(msg + strlen(msg) - 1) = '\0';
		snprintf(logdata->buffer, LOGLINE_MAXLEN, "%s %c * %s %s",
			 timestamp(), dir, from, msg + 8);
		free(msg);
	} else {
		snprintf(logdata->buffer, LOGLINE_MAXLEN, "%s %c %s: %s",
			 timestamp(), dir, from, message);
	}
...
{
	snprintf(logdata->buffer, LOGLINE_MAXLEN, "%s -!- Topic for %s: %s",
			 ...
			 const char *when)
{
	...
	snprintf(logdata->buffer, LOGLINE_MAXLEN, "%s -!- Topic set by %s [%s]",
		 timestamp(), who, timestr);
	free(timestr);
	log_write(logdata, channel, logdata->buffer);
}
